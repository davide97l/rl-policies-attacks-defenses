...
parser.add_argument(
    '--env-config',
    default='defended_models/radial_dqn/config.json',
    metavar='EC',
    help='environment to crop and resize info (default: defended_models/radial_dqn/config.json)')
parser.add_argument(
    '--save-max',
    default=True,
    metavar='SM',
...
# python atari_radial_dqn.py --task "PongNoFrameskip-v4"
if __name__ == '__main__':
    ...
    args.save_model_dir = os.path.join(args.save_model_dir, args.task, "radial_dqn")
    ...
    for i in range(args.epoch):
        r = record_game(curr_model, env, args)
        reward += r
    print("Evaluation using {} episodes: mean reward {:.5f}\n".format(args.epoch, reward / args.epoch))
